...
/* ------------------------------------------------------------------------------------------------------- */
     ...
     * где n = size из функций @refitem quickSort::sort() или @refitem quickSort::threeWaySort().
     */
    class quickSort {
...
/* ------------------------------------------------------------------------------------------------------- */
    /*! @brief Класс, реализующий алгоритм сортировки с помощью двоичной кучи (heap),
     * называемую также пирамидальной сортировкой - O(n*log(n)),
     * где n = size из функции @refitem heapSort::sort().
     */
     class heapSort {
         /* Для реализации данной сортировки нет необходимости реализовывать весь
          * функционал двоичной кучи, а тем более реализовывать двоичную кучу как отдельную
          * структуру данных (т.е. реализовывать очередь с приоритетом), поэтому реализуем только
          * часть, необходимую для, собственно, сортировки.
          * Напомним, что двоичная куча - это полное двоичное дерево, каждая вершина которого
          * не меньше (или не больше, или вообще может использоваться произвольный оператор сравнения)
          * чем любой из его вершин-детей. В данном случае используем сравнение "не меньше" для
          * родительской вершины (реализуем max-heap).
          * Двоичная куча может быть эффективно реализована через обычный массив, где элемент
          * a[0] - корень дерева, a[1] - его левая вершина-ребенок, a[2] - правая. Для любой вершины
          * с индексом i в массиве (в массивах с индексацией с нуля) его вершина-родитель - это
          * элемент с индексом floor((i-1)/2), левая вершина-ребенок - элемент с индексом 2i+1,
          * а правая - элемент с индексом 2i+2. */

         /*! Функция возвращает индекс вершины-родителя заданной вершины.
          *
          * @param index индекс заданной вершины в массиве, содержащем кучу.
          *
          * @return Индекс вершины-родителя заданной вершины с индексом index.
          */
         static std::size_t parent(std::size_t index) {
             return (index - 1) / 2;
         }

         /*! Функция возвращает индекс левой вершины-ребенка заданной вершины.
          *
          * @param index индекс заданной вершины в массиве, содержащем кучу.
          *
          * @return Индекс левой вершины-ребенка заданной вершины с индексом index.
          */
         static std::size_t leftChild(std::size_t index) {
             return 2 * index + 1;
         }

         /*! Функция возвращает индекс правой вершины-ребенка заданной вершины.
          *
          * @param index индекс заданной вершины в массиве, содержащем кучу.
          *
          * @return Индекс правой вершины-ребенка заданной вершины с индексом index.
          */
         static std::size_t rightChild(std::size_t index) {
             return 2 * index + 2;
         }

         /*! Функция осуществляет "просеивание вниз" элемента кучи с индексом index.
          * Предполагается, что при вызове этой функции элемент с индексом index расположен
          * в дереве (куче) не на своей позиции (т.е. нарушается свойство кучи), и потенциально
          * должен быть "опущен вниз" ближе к листьяем дерева, так как его значение потенциально
          * мало. Ассимптотическая сложность - O(log(size)).
          *
          * @tparam RandomAccessIterator - должен удовлетворять требованиям
          * Random Access Iterator.
          * @param heap - итератор на начало кучи (массива, хранящего кучу).
          * @param size - размер кучи.
          * @param index - индекс элемента в куче, требующего просеивания.
          */
         template<typename RandomAccessIterator>
         static void siftDown(RandomAccessIterator heap, std::size_t size, std::size_t index);

         /*! Функция превращает заданный массив в кучу (т.е. переставляет элементы
          * массива так, чтобы они удовлетворяли свойствам двоичной кучи).
          * Ассимптотическая сложность - O(size).
          *
          * @tparam RandomAccessIterator - должен удовлетворять требованиям
          * Random Access Iterator.
          * @param array - итератор на начало последовательности.
          ...
          */
         template<typename RandomAccessIterator>
         static void makeHeap(RandomAccessIterator array, std::size_t size);

     public:
         /*! Основная функция, реализующая сортировку с помощью двоичной кучи
          * (пирамидальную сортировку), имеющую ассимптотику O(size * log(size))
          * в худшем случае.
          *
          * @tparam RandomAccessIterator  - должен удовлетворять требованиям
          * Random Access Iterator.
...
/* ------------------------------------------------------------------------------------------------------- */
    template<typename RandomAccessIterator>
    void heapSort::siftDown(RandomAccessIterator heap, std::size_t size, std::size_t index) {
        /* Необходимо "опустить вниз" ближе к листьям элемент кучи с индексом index.
         * Будем использовать цикл, на каждой итерации которого, если потребуется,
         * будем менять местами элемент с индексом index и одну из его вершин-детей
         * (ту, что больше, чтобы свойство кучи однозначно выполнялось).
         *
         * Индекс максимального элемента на каждой итерации (среди родителя и двух
         * вершин-детей): */
        std::size_t indexOfMax,
        /* Индекс для текущей вершины-ребенка: */
                    childIndex;

        while (index < size) {
            indexOfMax = index;
            /* Выбираем максимум среди вершины-родителя и
             * вершин-детей: */
            childIndex = leftChild(index);
            if (childIndex < size && heap[childIndex] > heap[indexOfMax])
                indexOfMax = childIndex;

            childIndex = rightChild(index);
            if (childIndex < size && heap[childIndex] > heap[indexOfMax])
                indexOfMax = childIndex;

            /* Если "просеиваемый" элемент с индексом index не максимален, то меняем его
             * местами с бОльшей найденной вершиной-ребенком и продолжаем цикл: */
            if (index != indexOfMax) {
                std::swap(heap[index], heap[indexOfMax]);
                /* Присваеваем index новое значение, равное новой позиции
                 * "просеиваемого" элемента: */
                index = indexOfMax;
            } else
                /* Иначе "просеиваемый" элемент находится на своей позиции,
                 * завершаем цикл и алгоритм: */
                break;
        }
    }
...
/* ------------------------------------------------------------------------------------------------------- */
    template<typename RandomAccessIterator>
    void heapSort::makeHeap(RandomAccessIterator array, std::size_t size) {
        /* Если считать, что массив array - это куча, тогда ceil(size/2)
         * элементов в конце массива (элементы с бОльшими индексами) - это
         * листья дерева. А так как у листьев нет вершин-детей, то для всех
         * поддеревьев, у которых корень - это лист дерева (т.е. всех поддеревьев,
         * состоящий только из одной вершины-листа) свойства кучи тривиально выполняются.
         * Поэтому необходимо поставить на свои места только первые floor(size/2) элементов
         * массива. Для этого будем выполнять "просеивание вниз" всех этих элементов, начиная
         * с floor(size/2). Тогда, при просеивании каждого следующего элемента он будет потенциально
         * опускаться вниз в сторону поддеревьев, которые уже удовлетворяют свойствам кучи, поэтому
         * просеивание будет работать корректно. */
        for (std::size_t i = size / 2; i > 0; --i)
            siftDown(array, size, i);

        /* Чтобы не вызвать вычитания из нуля при использовании беззнакового типа
         * мы не обработали самый первый элемент массива с i = 0, сделаем это: */
        siftDown(array, size, 0);
    }

/* ------------------------------------------------------------------------------------------------------- */
    template<typename RandomAccessIterator>
    void heapSort::sort(RandomAccessIterator array, std::size_t size) {
        /* Сначала преобразуем массив array в кучу: */
        makeHeap(array, size);

        /* Теперь, по свойству кучи (max heap, если точнее), первый элемент массива (корень кучи) -
         * это максимальный элемент в массиве (куче). Обменяем его с последним
         * элементом массива (с последним листом кучи), тем самым поместив максимальный
         * элемент массива в конец, что и требуется при сортировке. Максимальный элемент
         * массива стоит на своей финальной позиции, поэтому можем уменьшить размер обрабатываемой
         * части массива на единицу. При этом необходимо восстановить свойства кучи, просеяв новый корень
         * (лист, который мы поместили на место корня) вниз, чтобы получить на вершине кучи следующий
         * максимальный элемент.
         * Проделав описанную выше операцию (size - 1) раз, мы получим отсортированный массив: */
        for (std::size_t i = 0, end = size - 1; i < end; ++i) {
            /* Меняем местами корень и последний лист: */
